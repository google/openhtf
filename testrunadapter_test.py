...
class TestRunAdapterBasics(googletest.TestCase):
  ...
  def setUp(self):
    self.adapter = testrunadapter_stub.StubTestRunAdapter()
  ...
  def testSettersAndGetters(self):
    ...
    self.assertEqual(testrunadapter_stub.TARGET_NAME,
...
class TestRunAdapterWithParameters(googletest.TestCase):
  ...
  def CreateAdapterWithSomeParameters(self):
    ...
    parameters.Add('name').Number().InRange(0, 1)
    return testrunadapter_stub.StubTestRunAdapter(parameters=parameters)
  ...
  def testConfigSaved(self):
    parameters = xtfparameters.TestParameterList()
    adapter = testrunadapter_stub.StubTestRunAdapter(parameters=parameters)
