...
class TestRunAdapter(object):
  ...
  def __init__(self, cell_number, test):
    ...
    xtf_test_run.test_status = test_runs_pb2.CREATED
    ...
    self._logger = xtflogger.XTFLogger(xtf_test_run)
  ...
  def GetCellLogger(self):
    """Returns a log adapter that logs to the xtf test run proto.

    Returns:
      xtflogger Adapter specific to this test run and cell.
    """
    return self._logger.GetLogAdapter(_LOG.getChild(str(self.cell)))
  ...
  def RecordPhaseTiming(self, phase_info):
    timing = self._xtf_test_run.timings.add()
    timing.name = phase_info.name
  ...
  def SetTestRunStatus(self, status):
    ...
    Args:
      status: An instance of test_runs_pb2.Status to be stored and forwarded.
    """
  ...
  def combined_parameter_status(self):
    ...
    if any(parameter.status != test_runs_pb2.PASS
           for parameter in self._xtf_test_run.test_parameters):
      return test_runs_pb2.FAIL
    return test_runs_pb2.PASS
  ...
  def AddConfigToTestRun(self, config=None):
    ...
    except xtfparameters.NotAParameterError:
      self.logger.warning(
  ...
  def PersistTestRun(self):
    ...
    logger = self.GetCellLogger()
    if result:
      logger.info('Log persisted to disk.')
    else:
      logger.error('Unable to save log to disk, halting test.')
    return result
