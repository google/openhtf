...
class MeasurementCollection(object):
  """Creates a collection of measurements.

  This collection can have masurement values retrieved and set via getters and
  ...
  def __init__(self, measurements):
    ...
    Args:
      measurements: A dictionary of name -> Measurement with
  ...
  def StatusOf(self, measurement_name):
    """Returns the status of a measurement.

    Args:
      measurement_name: The name of a measurement.
    Returns:
      The measurement's status
    Raises:
      MeasurementStatusError: Iff the measurement has no status.
    """
    measurement = self._measurements[measurement_name].measurement
    if measurement.HasField('status'):
      return measurement.status
    raise MeasurementStatusError(
        'Measurement %s has no status.' % measurement_name)

# TODO(jethier): Pick up the thread here.
...
def add(measurements):
  """Decorator to attaches the measurement(s) to the decorated phase.

  Args:
    measurements: List of MeasurementDescriptor objects to attach, or a single
    MeasurementDescriptor object to attach, or a string name from which to
    create a MeasurementDescriptor.
  Returns:
    A decorator that attaches the given measurement(s) to the decorated phase.
  """
  if isinstance(measurements, MeasurementDescriptor):
    measurements = [measurements]
  elif isinstance(measurements, str):
    measurements = [MeasurementDescriptor(measurements)]
  def decorate(phase):
    if not hasattr(phase, 'measurements'):
      phase.measurements = MeasurementCollection()
    for meas in measurements:
      phase.measurements.TrackMeasurement(meas)
    return phase
  return decorate
