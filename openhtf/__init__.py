...
# are provided (no conf value is available) when Test.Execute() is called, then
# we'll fall back to the machine's hostname.
...
class Test(object):
  ...
  def __init__(self, *phases, **metadata):
    ...
    self.loop = False
    self.phases = [PhaseInfo.WrapOrCopy(phase) for phase in phases]
  ...
  def Execute(self, loop=None, test_start=triggers.AutoStart,
    ...
    # If not provided via any config means by now, we default to hostname.
    if 'station_id' not in conf:
      conf.Load(station_id=socket.gethostname())
    ...
    server.Stop()
    return
>>>>>>> 51779d4... load --config-file on import, default station_id to hostname
