"""Interface to XTF configuration files.

XTF configuration files contain values which are specific to a single tester.
...
flags.DEFINE_string('openxtf_config',
                    '/usr/local/xtf_client/config/client.yaml',
                    'The XTF configuration file for this tester')

flags.DEFINE_multistring(
    'openxtf_config_value', [], 'Allows specifying a configuration key=value '
...
class _DeclaredParameters(object):
  ...
  This object is basically a helper for XTFConfig.  It provides locked access to
...
class ConfigModel(object):
  ...
  def Load(self, config_file=None, force_reload=False,
    ...
    Args:
      config_file: The file name to load configuration parameters from.
          Defaults to FLAGS.openxtf_config.
    ...
    try:
      filename = config_file or FLAGS.openxtf_config
      ...
      for keyval in FLAGS.openxtf_config_value:
        k, v = keyval.split('=')
        self._state[k] = v
  ...
  def Declare(self, name, description=None, **kwargs):
    ...
    As of XTF2 all configuration variables must be declared.  If a parameter is
...
class XTFConfig(object):
  """The configuration read from the XTF config file, or populated directly.
  ...
  def __init__(self, model=None):
    ...
    Args:
      model: The data model to use, defaults to the one shared amonst all config
          objects.
    """
    self.model = model or XTFConfig.model
  ...
  def __repr__(self):
    return '<XTFConfig (loaded: %s): 0x%x>' % (self.model.loaded, id(self))
  ...
  def CreateStackedConfig(self, model):
    ...
    Args:
      model: A ConfigModel instance or a dict of values that can be converted
          ...
          object will be used.
    Returns:
      A new StackedXTFConfig instance with model superseding the current model.
    """
    if not isinstance(model, ConfigModel):
      model = ConfigModel(state=model, declarations=self.model.declarations)
    return StackedXTFConfig([self.model, model])


class StackedXTFConfig(XTFConfig):  # pylint: disable=incomplete-protocol
  """Stacked version of XTFConfig.

  This is a layered (or stacked) XTFConfig that allows users to make one set of
  ...
  def __init__(self, models=(XTFConfig.model,)):
  ...
  def CreateStackedConfig(self, model):
    ...
    Returns:
      A new StackedXTFConfig instance with model superseding the current models.
    """
    if not isinstance(model, ConfigModel):
      model = ConfigModel(
          state=model, declarations=self._models[0].declarations)
    return StackedXTFConfig(self._models + [model])
  ...
  def __str__(self):
    return '<StackedXTFConfig (loaded: %s): 0x%x>' % (self.loaded, id(self))
  __repr__ = __str__
...
class ConfigValue(object):
  ...
  def __init__(self, config_key, config=None, value_fn=None):
    self.config = config or XTFConfig()
...
def InjectPositionalArgs(method):
  ...
  def MethodWrapper(**kwargs):
    """Wrapper that pulls values from the XTFConfig() for parameters."""
    config = XTFConfig()
...
Declare = XTFConfig().model.Declare
Load = XTFConfig().model.Load
LoadMissingFromDict = XTFConfig().model.LoadMissingFromDict
LoadFromDict = XTFConfig().model.LoadFromDict
Reset = XTFConfig().model.Reset

# Everywhere that uses configuration uses this, so we just declare it here.
Declare('target_name', 'The name of this tester')
