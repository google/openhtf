...
class PlugsTest(test.TestCase):
  ...
  def test_initialize(self):
    ...
    self.assertIs(
        ...
        self.plug_manager.provide_plugs((('adder_plug', AdderPlug),))[
            'adder_plug'
        ],
    )
  ...
  def test_plug_updates(self):
    self.plug_manager.initialize_plugs({AdderPlug})
    adder_plug_name = AdderPlug.__module__ + '.AdderPlug'
    update = self.plug_manager.wait_for_plug_update(adder_plug_name, {}, 0.001)
    self.assertEqual({'number': 0}, update)
    # No update since last time, this should time out (return None).
    self.assertIsNone(
        self.plug_manager.wait_for_plug_update(adder_plug_name, update, 0.001)
    )
    ...
    self.assertEqual(
        {'number': 1},
        self.plug_manager.wait_for_plug_update(adder_plug_name, update, 5),
    )
