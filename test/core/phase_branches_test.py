...
phase0, phase1, phase2, phase3 = _fake_phases(
...
class PhaseFailureCheckpointIntegrationTest(htf_test.TestCase):
            ...
            test_record.CheckpointRecord(
                ...
                result=phase_executor.PhaseExecutionOutcome(
                    phase_executor.ExceptionInfo(
                        phase_branches.NoPhasesFoundError, mock.ANY, mock.ANY
                    )
                ),
  ...
  def test_last__no_failures(self):
    ...
    self.assertPhasesOutcomeByName(
        test_record.PhaseOutcome.PASS, test_rec, 'phase0', 'phase1'
    )
  ...
  def test_last__failure_too_early(self):
    test_rec = yield htf.Test(
        fail_phase,
        phase0,
        phase_branches.PhaseFailureCheckpoint.last('last_early_fail'),
        phase1,
    )
    ...
    self.assertPhasesOutcomeByName(
        test_record.PhaseOutcome.FAIL, test_rec, 'fail_phase'
    )
  ...
  def test_last__failure_too_late(self):
    test_rec = yield htf.Test(
        phase0,
        phase_branches.PhaseFailureCheckpoint.last('last_late_fail'),
        fail_phase,
        phase1,
    )
  ...
  def test_last__failure(self):
    test_rec = yield htf.Test(
        phase0,
        fail_phase,
        phase_branches.PhaseFailureCheckpoint.last('last_fail'),
        error_phase,
    )

    self.assertTestFail(test_rec)
    self.assertPhasesOutcomeByName(
        test_record.PhaseOutcome.PASS, test_rec, 'phase0'
    )
    self.assertPhasesOutcomeByName(
        test_record.PhaseOutcome.FAIL, test_rec, 'fail_phase'
    )
  ...
  def test_last_fail_subtest__not_in_subtest(self):
    test_rec = yield htf.Test(
        fail_phase,
        phase_branches.PhaseFailureCheckpoint.last(
            'last_subtest', action=htf.PhaseResult.FAIL_SUBTEST
        ),
        error_phase,
    )
    ...
    self.assertEqual(
        [
            test_record.CheckpointRecord(
                ...
                result=phase_executor.PhaseExecutionOutcome(
                    phase_executor.ExceptionInfo(
                        phase_executor.InvalidPhaseResultError,
                        mock.ANY,
                        mock.ANY,
                    )
                ),
  ...
  def test_last_fail_subtest__fail_in_subtest(self):
    ...
    self.assertTestFail(test_rec)
    self.assertPhasesOutcomeByName(
        test_record.PhaseOutcome.PASS, test_rec, 'phase0', 'phase1', 'phase2'
    )
    self.assertPhasesOutcomeByName(
        test_record.PhaseOutcome.SKIP, test_rec, 'skip0'
    )
    self.assertPhasesOutcomeByName(
        test_record.PhaseOutcome.FAIL, test_rec, 'fail_phase'
    )
  ...
  def test_subtest_previous_fail__fail(self):
    test_rec = yield htf.Test(
        ...
        phase_branches.PhaseFailureCheckpoint.subtest_previous(
            'subtest_previous_fail', action=htf.PhaseResult.STOP
        ),
        error_phase,
    )
  ...
  def test_subtest_previous_fail__pass(self):
    test_rec = yield htf.Test(
        ...
        phase1,
        phase_branches.PhaseFailureCheckpoint.subtest_previous(
            'subtest_previous_pass', action=htf.PhaseResult.STOP
        ),
        phase2,
    )
  ...
  def test_all__pass(self):
    test_rec = yield htf.Test(
        phase0,
        phase_branches.PhaseFailureCheckpoint.all_previous('all_pass'),
        phase1,
    )
