...
class TestManager(object):
  ...
  _PHASE_RESULT_TO_CELL_STATE = {
      xtftest.PhaseResults.CONTINUE: xtf_pb2.WAITING,
      xtftest.PhaseResults.REPEAT: xtf_pb2.WAITING,
      xtftest.PhaseResults.FAIL: xtf_pb2.FAIL,
      xtftest.PhaseResults.TIMEOUT: xtf_pb2.TIMEOUT,
  }

  _ERROR_STATES = {xtf_pb2.TIMEOUT, xtf_pb2.ERROR}
  _FINISHED_STATES = {xtf_pb2.PASS, xtf_pb2.FAIL} | _ERROR_STATES
  ...
  def _PhaseResultToState(self, result):
    ...
    Returns:
      xtf_pb2.Status instance matching the result.
    """
  ...
  def _RecordTestFinish(self, state):
    ...
    if not self._test_run_adapter.serial:
      raise BlankDUTSerialError(
          'Blank or missing DUT serial, XTF requires a non-blank serial.')
    ...
    if not self._test_run_adapter.PersistTestRun():
      ...
      self._test_run_adapter.SetTestRunStatus(xtf_pb2.ERROR)
  ...
  def _SetFailureCodesBasedOnState(self, state):
    ...
    """
    if state == xtf_pb2.TIMEOUT:
      code = xtf_pb2.Status.Name(state)
      self._test_run_adapter.AddFailureCode(code=code)
    elif all(param.status == xtf_pb2.ERROR
      ...
      for parameter in self._test_run_adapter.xtf_test_run.test_parameters:
        if parameter.status != xtf_pb2.FAIL:
          continue
