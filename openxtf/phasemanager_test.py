...
class Phases(object):
  ...
  def NoCapabilitiesTimesout(cls, unused_test):
    try:
      with cls.lock:
        time.sleep(0.2)
        BusyLoopForTimeout()
  ...
  def NoCapabilitiesSlow(cls, unused_test):
    with cls.lock:
      try:
        Phases.event.set()
        time.sleep(0.2)
  ...
  def RunIfTrue(unused_test):
    pass

  @staticmethod
  @xtftest.TestPhase(run_if=lambda _: False)
  def RunIfFalse(unused_test):
  ...
  @staticmethod
  def BlacklistPhase(unused_test):
...
class PhasemanagerTest(googletest.TestCase):
  ...
  def CreatePhaseManagerForPhases(self, phases=None, model=None):
    testrun_adapter = openxtf_stubs.StubTestRunAdapter(phases)
  ...
  def testBlacklist(self):
    phases, phase_manager = self.CreatePhaseManagerForPhases([
        Phases.BlacklistPhase], model={'blacklist_phases': ['BlacklistPhase']})
