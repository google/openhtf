  ...
  class ParameterInstance(object):
    ...
    def __init__(self, test_run, descriptor):
      if not descriptor.optional:
        ...
        self._parameter = test_runs_pb2.TestParameter()
      ...
      self._parameter.status = test_runs_pb2.ERROR
    ...
    def SetValue(self, value):
      ...
      self._parameter.status = (
          test_runs_pb2.PASS if result else test_runs_pb2.FAIL)
...
class TextParameterDescriptor(data.BaseDataDescriptor):
  name = 'Text'
  info_tag = test_runs_pb2.TEXT

  def Transform(self, value):
    return str(value).decode('utf8', 'replace')


@ExtendedParameterDescriptor.AddDataDescriptor
class JpgParameterDescriptor(data.BaseDataDescriptor):
  name = 'JPG'
  info_tag = test_runs_pb2.JPG
...
class TimeseriesParameterDescriptor(data.BaseDataDescriptor):
  name = 'Timeseries'
  info_tag = test_runs_pb2.TIMESERIES
  Transform = staticmethod(str)  # pylint: disable=invalid-name
...
class ExtendedSetValueCapability(object):
  ...
  def SetValueText(desc, parameter, value):
    parameter.value = desc.Transform(value)
...
class ParameterCollection(object):
  ...
  def IsParameterPassing(self, name):
    ...
    return status == test_runs_pb2.PASS
...
def GetUomFromUnitCode(unit_code):
  ...
  """
  uom_code = (test_runs_pb2.Units.UnitCode.DESCRIPTOR.values_by_number[
      unit_code].GetOptions().Extensions[test_runs_pb2.uom_code])
  if uom_code:
    return uom_code
  return None
