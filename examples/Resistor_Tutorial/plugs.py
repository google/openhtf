...
# You can check the device ID with psu.query("*IDN?")

class MultimeterPlug(BasePlug):
    """Plug for control of a multimeter with pyvisa.
    ...
    def read_current(self) -> Tuple[float]:
        ...
        if not self.simulate:
            current = self.multimeter.query(
                "MEASure:CURRent:DC? AUTO,1E-3"
                )[:-1]
        else:
    ...
    def read_voltage(self) -> Tuple[float]:
        ...
        if not self.simulate:
            voltage = self.multimeter.query(
                "MEASure:VOLTage:DC? 10,1E-3"
                )[:-1]
        else:
