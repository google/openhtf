"""Tests for google3.googlex.glass.xtf.openxtf.openxtf.xtfparameters."""
...
from google3.googlex.glass.xtf.shared import data
...
class ParameterCollectionTest(googletest.TestCase):
  ...
  def setUp(self):
    self.params = xtfparameters.ParameterCollection({
  ...
  def testInvalidParameter(self):
    with self.assertRaises(xtfparameters.NotAParameterError):
  ...
  def testGetStatusRaisesOnExtended(self):
    with self.assertRaises(xtfparameters.TestParameterError):
...
class TestParameterListTest(googletest.TestCase):
  ...
  def setUp(self):
    ...
    self.parameters = xtfparameters.TestParameterList()

  def testUnion(self):
    self.parameters.Add('name').Number().InRange(0, 1)

    parameters2 = xtfparameters.TestParameterList()
    parameters2.Add('name2').Number().InRange(0, 1)

    parameters3 = xtfparameters.TestParameterList()
    parameters3.Add('name3').Number().InRange(0, 1)

    merged = xtfparameters.TestParameterList.Union(
  ...
  def testUnionRaisesForDuplicates(self):
    ...
    with self.assertRaises(xtfparameters.DuplicateTestParameterError):
      xtfparameters.TestParameterList.Union(self.parameters, parameters2)
  ...
  def testCreateInstance(self):
    ...
    parameters = frozenset(self.parameters.parameters)
    self.assertIn(string1, parameters)
    self.assertIn(number1, parameters)
    self.assertIn(info1, parameters)
...
class ParameterDefinitionTest(googletest.TestCase):
  ...
  def testDescriptionSetter(self):
    ...
    binder = xtfparameters.TestParameterDescriptor(
  ...
  def testRaisesIfPassFailIsNotReturnedFromVerifier(self):
    ...
    @xtfparameters.TestParameterDescriptor.AddDataDescriptor
    class BadDescriptor(data.BaseDataDescriptor):
      name = 'Bad'

    binder = xtfparameters.TestParameterDescriptor('name', parameter_tag='tag')
  ...
  def testBooleanParameterLogsSuccessfully(self):
    binder = xtfparameters.TestParameterDescriptor('name')
  ...
  def testNumericParameterNoValidator(self):
    binder = xtfparameters.TestParameterDescriptor('name').Number()
    prop = binder.AddToTestRun(self.test_run)
  ...
  def testTextParameterNoValidator(self):
    binder = xtfparameters.TestParameterDescriptor('name').String()
    prop = binder.AddToTestRun(self.test_run)
  ...
  def testOptionalParameters(self):
    binder = xtfparameters.TestParameterDescriptor('name', optional=True)
    binder.Number().InRange(1, 3)
    prop = binder.AddToTestRun(self.test_run)
  ...
  def testWrappedParameterIsAddedToTestRun(self):
    binder = xtfparameters.TestParameterDescriptor('name').Number().Matches(1)
...
class ExtendedParameterTest(googletest.TestCase):
  ...
  def testDescriptionSetter(self):
    description = """A parameter which is awesome
    No really seriously awesome."""
    binder = xtfparameters.ExtendedParameterDescriptor(
  ...
  def testBasicParameterLogged(self):
    binder = xtfparameters.ExtendedParameterDescriptor('name').Text()
    prop = binder.AddToTestRun(self.test_run)

    prop.SetValue('value')
    self.assertEquals(prop.parameter.name, 'name')
    self.assertEquals(prop.parameter.type, test_runs_pb2.TEXT)
  ...
  def testBase64EnablesForJPG(self):
    desc_maker = xtfparameters.ExtendedParameterDescriptor
...
class TestParamInstanceInTestRun(googletest.TestCase):
  ...
  def setUp(self):
    configuration.XTFConfig.model.LoadFromDict({})
    self.addCleanup(configuration.XTFConfig.model.Reset)
    self.parameter_list = xtfparameters.TestParameterList()
  ...
  def testUnitCodePersisted(self):
    ...
    self.assertEquals(
        'MMT', xtfparameters.GetUomFromUnitCode(
            self.test_run.test_parameters[0].unit_code))
  ...
  def testNoUomCodePersisted(self):
    ...
    self.assertEquals(None,
                      xtfparameters.GetUomFromUnitCode(
                          self.test_run.test_parameters[0].unit_code))
  ...
  def testParameterInstance(self):
    ...
    parameters = self.parameter_list.ForTestRun(self.test_run)

    parameters.hello = 'not-goodbye'
    parameters.info = 'goodbye'

    self.assertEquals('not-goodbye', parameters.hello)
  ...
  def testParameterSetAndGetItem(self):
    ...
    parameters['info_setitem'] = 'really-bye'
    self.assertEquals(parameters['info_setitem'], 'really-bye')
...
class XtfparametersTest(googletest.TestCase):
  ...
  def testBasics(self):
    param = xtfparameters.TestParameterList().Add(
